...
function advancedPagerHtml(s) {
  ...
  return `<div class="pagination"><button class="btn small" ${s.page <= 1 ? 'disabled' : ''} onclick="loadAdvanced(${s.page - 1})">Previous</button><span>Page ${s.page} / ${totalPages}, Total ${s.total}</span><button class="btn small" ${s.page >= totalPages ? 'disabled' : ''} onclick="loadAdvanced(${s.page + 1})">Next</button>${pageJumpHtml('pageJumpAdvanced','advanced',totalPages)}</div>`;
}
